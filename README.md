...
- [ ] Explain what a token is used for. A token is used to authenticate users. Tokens allow users to access protected components. 
- [ ] What steps can you take in your web apps to keep your data secure? Can make sure that tokens are required for access, make the underlying logic of the app to be in sync with the backend architecture.
- [ ] Describe how web servers work. Web servers processes network requests via HTTP. Web servers store, process, and deliver web pages to clients. 
- [ ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers. POST, GET, PUT, and DELETE
